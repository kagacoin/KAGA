    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of KAGA earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 KAGA");
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("KAGA")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair KAGA
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(177,178,179));
	set1->setColor(QColor(126,127,128));
